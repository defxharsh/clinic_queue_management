...
async function bookAppointment() {
  ...
  // ── HIGH PRIORITY FIX ──
  // Sort: high priority first, then by insertion order
  // We assign a stable insertion index before sorting
  queue.forEach((p, i) => p._insertIdx = i);

  queue.sort((a, b) => {
    const aHigh = a.priority === 'High' ? 0 : 1;
    const bHigh = b.priority === 'High' ? 0 : 1;
    if (aHigh !== bHigh) return aHigh - bHigh; // high comes first
    return a._insertIdx - b._insertIdx;         // preserve order within same priority
  });

  // Re-assign queue numbers after sort
  queue.forEach((p, i) => {
    p.qno = i + 1;
    delete p._insertIdx;
  });

  // Find the REAL position of the newly added patient AFTER sorting
  const realPosition = queue.findIndex(
    q => q.name === name && q.email === email && q.sym === sym
  ) + 1;
  ...
  toast(
    `✓ Booked! You are #${realPosition} in queue.${priority === 'High' ? ' (High priority — moved up!)' : ''} Est. wait: ~${estWait} min.`,
    'success'
  );
